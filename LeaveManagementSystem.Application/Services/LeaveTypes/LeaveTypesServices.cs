...
public class LeaveTypesServices : ILeaveTypesServices
{
    ...
    private readonly ILogger<LeaveTypesServices> _logger;

    public LeaveTypesServices(ApplicationDbContext context, ILogger<LeaveTypesServices> logger)
    {
        _context = context;
    ...
    {
        _logger.LogInformation("Creating a new leave type with name: {LeaveTypeName} and days: {Days}", leaveTypeVM.Name, leaveTypeVM.Days);
        var leaveTypeDM = new LeaveType
        {
            LeaveTypeName = leaveTypeVM.Name,
            NumberOfDays = leaveTypeVM.Days
